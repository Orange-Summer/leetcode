...
//loc.free(2); // 释放 mID 为 2 的所有内存单元。内存数组变为 [1, ,3, , , , , , , ] 。返回 1 ，因为只有 1 
//个 mID 为 2 的内存单元。
//loc.allocate(3, 4); // 最左侧的块的第一个下标是 3 。内存数组变为 [1, ,3,4,4,4, , , , ]。返回 3 。
//loc.allocate(1, 1); // 最左侧的块的第一个下标是 1 。内存数组变为 [1,1,3,4,4,4, , , , ]。返回 1 。
//loc.allocate(1, 1); // 最左侧的块的第一个下标是 6 。内存数组变为 [1,1,3,4,4,4,1, , , ]。返回 6 。
//loc.free(1); // 释放 mID 为 1 的所有内存单元。内存数组变为 [ , ,3,4,4,4, , , , ] 。返回 3 ，因为有 3 个
// mID 为 1 的内存单元。
...
// 
//
// Related Topics 设计 数组 哈希表 模拟 👍 14 👎 0
...
//Java：设计内存分配器
public class P2502DesignMemoryAllocator {
    public static void main(String[] args) {
        // Solution solution = new P2502DesignMemoryAllocator().new Solution();
        // TO TEST
        Allocator allocator = new P2502DesignMemoryAllocator().new Allocator(10);
    ...
    class Allocator {
        ...
        public int allocate(int size, int mID) {
            ...
            for (int i = 0; i < this.size; i++) {
                ...
                if (free >= size) {
                    Arrays.fill(this.memory, i - size + 1, i + 1, mID);
                    return i - size + 1;
                }
            }
            return -1;
