...
public class P599MinimumIndexSumOfTwoLists {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(solution.findRestaurant(new String[]{"Shogun", "Tapioca Express", "Burger King", "KFC"}, new String[]{"KFC", "Shogun", "Burger King"})));
        // TO TEST
    }
        ...
        public String[] findRestaurant(String[] list1, String[] list2) {
            ...
            for (int i = 0; i < list2.length; i++) {
                if (map.containsKey(list2[i])) {
                    if (map.get(list2[i]) + i < sumIndex) {
                        tem.clear();
                        tem.add(list2[i]);
                        sumIndex = map.get(list2[i]) + i;
                    } else if (map.get(list2[i]) + i == sumIndex) {
                        tem.add(list2[i]);
                    }
                }
            }
            return tem.toArray(new String[0]);
        }
    }
